	...
	public partial class Form1 : Form
	{
		...
		*/
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			if (INI.Read("Form.Pos", "Y") != "")									// параметр найден
			{
				form_top = Convert.ToInt16(INI.Read("Form.Pos", "Y"));
			}
			else
			{
				form_top = 0;														// параметр не найден
			}

			if (INI.Read("Form.Pos", "X") != "")                                    // параметр найден
			{
				form_left = Convert.ToInt16(INI.Read("Form.Pos", "X"));
			}
			else
			{
